 ...
 *
 * Props:
 *   target — numeric headline target ($120000 → "HOW DO I REACH $120,000+?"); set 0 to use customHeadline
 *   customHeadline — optional string to override the default headline (used for Strategic Partner)
 *   inputs — array of { name, label, min, max, step, default, unit, hint }
 *   calculate — function (values) => { rangeLow, rangeHigh, outputs:[{label,value}], hierarchyLabel? }
 *   rangeLabel — optional override for "Illustrative Annual Opportunity" label (e.g. Strategic Partner)
 */

export default function OpportunitySimulator({ target, customHeadline, inputs, calculate, rangeLabel }) {
  const [values, setValues] = useState(Object.fromEntries(inputs.map(i => [i.name, i.default])));
  ...
  const headline = customHeadline || (target > 0 ? `HOW DO I REACH $${target.toLocaleString()}+?` : 'INTERACTIVE OPPORTUNITY SIMULATOR');
  const rangeLbl = rangeLabel || 'Illustrative Annual Opportunity';
    ...
    <div className="bg-navy-800 border border-gold/30 rounded-2xl p-6 md:p-8 relative overflow-hidden">
      ...
      <div className="text-center mb-6">
        ...
        <h2 className="font-display text-3xl md:text-4xl font-medium tracking-tight mb-2">
          {customHeadline ? <em className="text-gold italic">{customHeadline}</em>
      ...
      {/* ROLE HIERARCHY (if provided) */}
      {result.hierarchyLabel && (
        <div className="bg-navy-950/50 border border-gold/15 rounded-xl px-4 py-2.5 mb-4 text-center">
          <p className="text-[9px] font-bold tracking-widest uppercase text-gold/80 mb-1">This Pathway Aggregates</p>
          <p className="text-[11px] text-surface-secondary leading-snug">{result.hierarchyLabel}</p>
        </div>
      )}

      {/* LIVE OPPORTUNITY OUTPUT */}
      <div className="bg-gold/[0.08] border border-gold/30 rounded-xl p-5 mb-6 text-center relative overflow-hidden">
        ...
        <p className="text-[9px] font-bold tracking-widest uppercase text-gold/80 mb-1">{rangeLbl}</p>
        ...
        <p className="text-[10px] text-surface-muted mt-2 italic">
          ...
          Confidential compensation details activate after recognition.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
        {inputs.map(input => (
          <SliderInput key={input.name} input={input} value={values[input.name]} onChange={v => update(input.name, v)} />
        ))}
      </div>
      ...
      <p className="text-[9px] text-surface-muted text-center mt-5 italic max-w-xl mx-auto">
        Simulator shows illustrative ecosystem scenarios only. Actual compensation, contracts, ranks, and payout systems are confidential and subject to written agreement, compliance, processor, legal, and tax review. No income is guaranteed.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <input type="range" min={input.min} max={input.max} step={input.step} value={value}
        onChange={e => onChange(Number(e.target.value))} className="w-full accent-gold cursor-pointer" />
